...
class CachePerformanceAnalyzer:
            ...
            if include_plots:
                html += """
                    ...
                    <div class="section">
                        ...
                        <div class="plot-container">
                            <h3><i class="fas fa-cubes"></i> Cluster Analysis</h3>
                            <img src="{}" alt="Cluster Analysis">
                        </div>
                    </div>
                """.format(
                    ...
                    plot_paths.get("cluster_analysis", "#")
                )
    ...
    def plot_cluster_analysis(self, run_data: Dict[str, Any], save_path: Optional[str] = None) -> plt.Figure:
        ...
        try:
            ...
            # Only plot if we have cluster data
            if len(cluster_counts) > 0:
                ...
                ax1.text(0.5, 0.5, "No cluster data to display", 
                        horizontalalignment='center', verticalalignment='center',
            ...
            for cluster_id, group in df.groupby('cluster_id'):
                if group.shape[0] > 0:  # Only process groups with data
                    hits = group[group['event_type'].str.contains('HIT', na=False)].shape[0]
                    hit_rate = hits / group.shape[0]
            ...
            if len(cluster_ids) > 0:
                ...
                ax2.text(0.5, 0.5, "No data to calculate hit rates", 
                        horizontalalignment='center', verticalalignment='center',
            ...
            # Only plot temperature data if it exists and has non-null values
            if 'temperature' in df.columns and df['temperature'].notna().any():
                # Filter out rows without temperature data
                temp_df = df[df['temperature'].notna()]
            
                # Only plot if we have data for at least one cluster
                if len(temp_df) > 0 and 'cluster_id' in temp_df.columns:
                    # Get unique clusters that have temperature data
                    clusters_with_temp = temp_df['cluster_id'].unique()
                
                    if len(clusters_with_temp) > 0:
                        # Filter the dataframe to only include clusters with temperature data
                        plot_df = temp_df[temp_df['cluster_id'].isin(clusters_with_temp)]
                    
                        # Create the boxplot
                        sns.boxplot(x='cluster_id', y='temperature', data=plot_df, ax=ax3)
                        ax3.set_title('Temperature Distribution by Cluster', fontsize=12)
                        ax3.set_xlabel('Cluster ID')
                        ax3.set_ylabel('Temperature')
                    else:
                        ax3.text(0.5, 0.5, "No temperature data per cluster", 
                                horizontalalignment='center', verticalalignment='center',
            ...
            # Only plot if we have response time data
            if 'response_time' in df.columns and df['response_time'].notna().any():
                # Filter out rows without response time data
                rt_df = df[df['response_time'].notna()]
            
                if len(rt_df) > 0 and 'cluster_id' in rt_df.columns:
                    # Get unique clusters that have response time data
                    clusters_with_rt = rt_df['cluster_id'].unique()
                
                    if len(clusters_with_rt) > 0:
                        # Filter the dataframe to only include clusters with response time data
                        plot_df = rt_df[rt_df['cluster_id'].isin(clusters_with_rt)]
                    
                        # Create the boxplot
                        sns.boxplot(x='cluster_id', y='response_time', data=plot_df, ax=ax4)
                        ax4.set_title('Response Time by Cluster', fontsize=12)
                        ax4.set_xlabel('Cluster ID')
                        ax4.set_ylabel('Response Time (s)')
                    else:
                        ax4.text(0.5, 0.5, "No response time data per cluster", 
                                horizontalalignment='center', verticalalignment='center',
                                transform=ax4.transAxes, fontsize=12)
                else:
                    ax4.text(0.5, 0.5, "No response time data available", 
                            horizontalalignment='center', verticalalignment='center',
                            transform=ax4.transAxes, fontsize=12)
            else:
                ax4.text(0.5, 0.5, "No response time data available", 
